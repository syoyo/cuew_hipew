...
#define CUBLAS_LIBRARY_FIND_CHECKED(name) \
        _LIBRARY_FIND_CHECKED(cublas_lib, name)
#define CUBLAS_LIBRARY_FIND(name) _LIBRARY_FIND(cublas_lib, name)
...
static int cuewCublasInit(void) {
  /* Library paths. */
#ifdef _WIN32
  /* Expected in c:/windows/system or similar, no path needed. */
  const char *cublas_paths[] = {"cublas.dll", NULL};
#else
  const char *cublas_paths[] = {"libcublas.so",
#  if defined(__x86_64__) || defined(_M_X64)
  ...
  /* Load library. */
  cublas_lib = dynamic_library_open_find(cublas_paths);
